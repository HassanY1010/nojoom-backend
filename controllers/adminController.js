...
async updateUser(req, res) {
    ...
    try {
      const [
        ...
        pool.execute(`
          SELECT 
            ...
            COUNT(CASE WHEN DATE(created_at) = CURDATE() THEN 1 END) as new_videos_today,
        ...
        pool.execute(`
          SELECT 
            ...
            COUNT(CASE WHEN DATE(created_at) = CURDATE() THEN 1 END) as broadcasts_today
          FROM broadcasts
        `)
      ]);
      ...
      res.json({
        stats: {
          ...
          videos: videosStats[0][0],
          messages: messagesStats[0][0],
          broadcasts: broadcastsStats[0][0]
        },
  ...
  try {
    ...
    // تحويل undefined إلى null
    maintenance_mode = maintenance_mode ?? null;
    chat_enabled = chat_enabled ?? null;
    upload_enabled = upload_enabled ?? null;
    max_video_size = max_video_size ?? null;
    max_video_duration = max_video_duration ?? null;
    allowed_video_formats = allowed_video_formats ?? null;

    const [result] = await pool.execute(
      `INSERT INTO system_settings 
       (id, maintenance_mode, chat_enabled, upload_enabled, max_video_size, max_video_duration, allowed_video_formats, updated_at) 
       VALUES (1, ?, ?, ?, ?, ?, ?, NOW())
       ON DUPLICATE KEY UPDATE
       maintenance_mode = VALUES(maintenance_mode),
       chat_enabled = VALUES(chat_enabled),
       upload_enabled = VALUES(upload_enabled),
       max_video_size = VALUES(max_video_size),
       max_video_duration = VALUES(max_video_duration),
       allowed_video_formats = VALUES(allowed_video_formats),
       updated_at = NOW()`,
      [maintenance_mode, chat_enabled, upload_enabled, max_video_size, max_video_duration, allowed_video_formats]
    );
