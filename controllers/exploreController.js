...
export const exploreController = {
  ...
  async getExploreVideos(req, res) {
    try {
      ...
      // استخدام COUNT(*) بدلاً من COUNT(DISTINCT l.id) لأن جدول likes لا يحتوي على id
      let query = `
        SELECT 
          v.*,
          u.username,
          ...
          EXISTS(SELECT 1 FROM likes WHERE video_id = v.id AND user_id = ?) as is_liked,
          EXISTS(SELECT 1 FROM follows WHERE following_id = v.user_id AND follower_id = ?) as is_following
        FROM videos v
      ...
      const queryParams = [userId || 0, userId || 0];
      ...
      switch (filter) {
        case 'trending':
          query += ` ORDER BY (COUNT(DISTINCT l.user_id) * 0.4 + COUNT(DISTINCT vw.id) * 0.3 + COUNT(DISTINCT c.id) * 0.3) DESC, v.created_at DESC`;
          break;
        case 'popular':
          query += ` ORDER BY COUNT(DISTINCT vw.id) DESC, COUNT(DISTINCT l.user_id) DESC`;
          break;
        case 'latest':
        ...
        default:
          if (userId) {
            // استخدام نظام التوصية إذا كان المستخدم مسجل الدخول
            try {
              ...
              // استمر بالترتيب الافتراضي في حالة خطأ النظام التوصية
              query += ` ORDER BY (COUNT(DISTINCT l.user_id) * 0.3 + COUNT(DISTINCT vw.id) * 0.7) DESC, v.created_at DESC`;
            }
          } else {
            // الترتيب الافتراضي للمستخدمين غير المسجلين
            query += ` ORDER BY (COUNT(DISTINCT l.user_id) * 0.3 + COUNT(DISTINCT vw.id) * 0.7) DESC, v.created_at DESC`;
          }
      }
      ...
      const formattedVideos = videos.map(video => ({
        ...
        is_chat_video: video.is_chat_video,
  ...
  async getExploreUsers(req, res) {
    try {
      ...
      // استخدام COUNT(*) بدلاً من COUNT(DISTINCT l.id)
      let query = `
        SELECT 
          u.*,
          ...
          COALESCE(SUM(v.duration), 0) as total_watch_time,
          EXISTS(SELECT 1 FROM follows WHERE following_id = u.id AND follower_id = ?) as is_following
        FROM users u
      ...
      query += ` 
        GROUP BY u.id
        ORDER BY 
          (COUNT(DISTINCT f1.follower_id) * 0.4 + 
           COUNT(DISTINCT l.user_id) * 0.3 + 
           COALESCE(SUM(v.views), 0) * 0.3) DESC,
          u.created_at DESC
        LIMIT ? OFFSET ?
      `;
