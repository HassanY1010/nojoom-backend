...
export class Message {
  ...
  static async updateActiveConversation(user1_id, user2_id, last_message_id, last_message_content) {
    ...
    await pool.execute(
      `INSERT INTO active_conversations 
       (user1_id, user2_id, last_message_id, last_message_content, last_message_at, unread_count) 
       VALUES (?, ?, ?, ?, NOW(), 
         (SELECT COUNT(*) FROM direct_messages 
          WHERE ((sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)) 
          AND is_read = FALSE AND receiver_id = ?))
       ON DUPLICATE KEY UPDATE 
         last_message_id = VALUES(last_message_id),
         last_message_content = VALUES(last_message_content),
         last_message_at = VALUES(last_message_at),
         unread_count = (SELECT COUNT(*) FROM direct_messages 
                        WHERE ((sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)) 
                        AND is_read = FALSE AND receiver_id = ?)`,
      [user1, user2, last_message_id, last_message_content, 
       user1_id, user2_id, user2_id, user1_id, user2_id,
       user1_id, user2_id, user2_id, user1_id, user2_id]
    );
  }
  ...
  static async markAsRead(senderId, receiverId) {
    ...
    // تحديث عدد الرسائل غير المقروءة في المحادثة النشطة
    await this.updateConversationUnreadCount(senderId, receiverId);
    
    return result.affectedRows;
  }

  // تحديث عدد الرسائل غير المقروءة في المحادثة
  static async updateConversationUnreadCount(user1_id, user2_id) {
    const [user1, user2] = [user1_id, user2_id].sort((a, b) => a - b);
    
    await pool.execute(
      `UPDATE active_conversations 
       SET unread_count = (SELECT COUNT(*) FROM direct_messages 
                          WHERE ((sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)) 
                          AND is_read = FALSE AND receiver_id = ?)
       WHERE user1_id = ? AND user2_id = ?`,
      [user1_id, user2_id, user2_id, user1_id, user2_id, user1, user2]
    );
  }
